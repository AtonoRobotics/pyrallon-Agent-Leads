...
The product must implement **Habitat** as the autonomous runtime for the Buyer Operations Agent. Habitat owns observation, work selection, context compilation, cognitive invocation, proposal evaluation, policy and authority evaluation, just-in-time effect admission, single-use `EffectPermit` issuance/redemption, idempotency registration with PostgreSQL, authority-decision evidence, semantic progression, replanning, run lifecycle, and continuation or sleep. Cognitive adapters propose typed work; Habitat admits or rejects external effects mechanically. Habitat does not own canonical business truth or connector provider truth; PostgreSQL and provider receipts remain authoritative in their respective domains.
...
Temporal is Habitat's replaceable durability substrate. It provides workflow signals, wakes, timers, workflow and activity leases, durable execution, workflow concurrency scheduling, retries, compensation, recovery, replay, and worker lifecycle for Habitat runs, but it does not independently select work, progress buyer semantics, own canonical business truth, or grant external-effect authority. Habitat may be hosted inside Temporal workflows or invoke Temporal through an adapter; Temporal must not become a second autonomous agent runtime. The connector gateway must reject every provider-changing call that lacks a current, single-use Habitat permit. The topology is:
