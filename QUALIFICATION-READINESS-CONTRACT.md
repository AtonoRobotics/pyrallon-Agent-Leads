...
The executable acceptance binding is `src/buyer_ops_contracts/contract_acceptance.py`; fixtures and deterministic tests are in `tests/fixtures/qualification_readiness` and `tests/test_new_contract_acceptance.py`. Canonical digests are `sha256:` followed by lowercase SHA-256 over UTF-8 JSON serialized with sorted keys, no insignificant whitespace, and no ASCII escaping. The input digest excludes `inputDigest` and the derived `inputSetId`. Production writer activation remains blocked as recorded in `QUALIFICATION-READINESS-COMPATIBILITY.json`.
