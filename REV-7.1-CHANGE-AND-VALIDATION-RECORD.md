...
|---|---|---|---|
| Habitat and Temporal ownership was incorrectly reduced to a kernel-versus-orchestrator split | Habitat now owns the autonomous loop, semantic progression, work selection, replanning, and run lifecycle; Temporal is Habitat's replaceable durability substrate for timers, signals, retries, replay, recovery, and worker execution | PRD FR-20, Design Ledger §5, OT-01 §4–§5, PKT-03/04 | Ownership search and cross-artifact semantic review |
...
## 5. Residual activation decisions and implementation-discovered defects

The original nine deployment decisions remain required at their stated activation boundaries:
...
PKT-00 through PKT-10 implementation audits identified OPEN-010 through OPEN-018 as governing
contract defects. SCP-01 resolved them on 2026-08-19 through ontology 0.3.0 and the versioned ingress,
cognitive policy, compensation, connector, release-activation, operator-surface, and telemetry/SLO
contracts. `SPECIFICATION-CLOSURE-PACKET.md` contains the controlling verification evidence.

## 6. Release condition

The design package remains the baseline, as revised by ontology 0.3.0, SCP-01, and the explicit ledger.
Live capability activation remains governed by `PRODUCTION-GATE-REGISTRY.yaml`, closure of relevant
open decisions and specification defects, production-equivalent provider evidence, and independent
completion reconstruction. OPEN-016 contract resolution does not itself authorize activation.
